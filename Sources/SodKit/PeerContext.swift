...
public struct PeerContext: Sendable {
    ...
    /// (not a unix socket, peer already gone) — callers then fall back to the generic prompt.
    ...
    /// KERN_PROCARGS2 layout: int32 argc, the exec path, NUL padding, then argc NUL-terminated
    /// argv strings (followed by the environment, which we don't read).
    private static func arguments(of pid: pid_t) -> [String] {
        ...
        let strings = raw[4..<size].split(separator: 0, omittingEmptySubsequences: true)
...
public func gitRepoName(cwd: String) -> String? {
