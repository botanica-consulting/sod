...
private func transact(socket path: String, _ request: Data) -> (type: UInt8, payload: Data) {
    ...
    guard let len = try? r.readUInt32(), len >= 1, Int(len) <= maxMessage,
        let body = readExactly(fd, Int(len))
    else { errExit("malformed response from agent") }
...
private func listIdentities(_ sock: String, full: Bool) {
    ...
    for _ in 0..<n {
        ...
        if full {
            print("ecdsa-sha2-nistp256 \(blob.base64EncodedString()) \(comment)")
        } else {
            print("256 \(sshFingerprint(blob)) \(comment) (ECDSA)")
        }
    }
}
...
public struct Add: ParsableCommand {
    ...
    @Option(
        name: .customShort("a"),
        help: ArgumentHelp("Agent socket (default $SSH_AUTH_SOCK, else ~/.ssh/sod-agent.sock).", valueName: "socket"))
    var socket: String?
