...
public struct Keygen: ParsableCommand {
    ...
    @Option(name: .customShort("f"), help: ArgumentHelp("Output keyfile (default ~/.ssh/id_sod).", valueName: "keyfile"))
    var file: String?

    @Option(name: .customShort("C"), help: ArgumentHelp("Comment for the public-key line.", valueName: "comment"))
    var comment: String?

    @Option(name: .customShort("t"), help: ArgumentHelp("Key type — only 'ecdsa' (the Secure Enclave is P-256).", valueName: "type"))
    var type: String?

    @Flag(name: .customShort("y"), help: "Read a handle file (-f) and print its public-key line.")
    var extractPublic = false

    @Option(name: .customShort("b"), help: ArgumentHelp("Unsupported (Secure Enclave is fixed at P-256).", visibility: .private))
    var bits: String?

    @Option(name: .customShort("N"), help: ArgumentHelp("Unsupported (Touch ID replaces the passphrase).", visibility: .private))
    ...
    public func run() throws {
        ...
        if fm.fileExists(atPath: path) {
            ...
            guard HandleFile.isHandleFile(existing) else {
                errExit("refusing to overwrite '\(path)': not a sod handle file " +
    ...
    private func extractPub(path: String) {
        ...
        guard backend.kind == decoded.kind else {
            errExit("handle is for a different backend (kind \(decoded.kind)); this build uses " +
