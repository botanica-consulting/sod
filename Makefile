...
universal: ## staged universal (arm64+x86_64) dist/sod
...
man: ## lint the man page
	@mandoc -Tlint man/sod.1 && echo "man: sod.1 OK"

sign: universal ## codesign dist/sod IFF $$DEVELOPER_ID_APP set
...
install: universal ## install to /usr/local (sudo)
	...
	sudo install -m 0755 $(BIN) /usr/local/bin/sod
	sudo install -m 0644 man/sod.1 /usr/local/share/man/man1/sod.1

uninstall: ## remove local install + optional LaunchAgent
	sudo rm -f /usr/local/bin/sod /usr/local/share/man/man1/sod.1
...
release: clean test universal sign pkg notarize ## full pipeline (signing/notary self-skip if env unset)
	...
	@ls -la dist/*.pkg dist/sod 2>/dev/null || true
